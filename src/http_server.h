...
// Максимальный размер HTTP-сообщения ограничен тем, сколько байт мы можем считать или отправить через сокет каждый раз
...
// Обработчик запросов должен ожидать запрос в качестве аргумента и возвращать ответ
using HttpRequestHandler_t = std::function<HttpResponse(const HttpRequest&)>;

// Сервер состоит из:
// - 1 основного потока
// - 1 потока прослушивателя, ответственного за прием новых соединений
// - Возможно, множество потоков, обрабатывающих HTTP-сообщения и взаимодействующих
// с клиентами через сокет.
//   Количество рабочих определяется константой
class HttpServer {
 public:
  ...
  void send_file(int sock_fd_, const std::string& filename);
