...
HttpMethod string_to_method(const std::string& method_string) {
  ...
  } else if (method_string_uppercase == "DOWNLOAD") {
    return HttpMethod::DOWNLOAD;
  } else if (method_string_uppercase == "SAVEAS") {
    return HttpMethod::SAVEAS;
  } else {
...
// bool HttpRequest::download(const Uri &uri, const std::string &local) const
// {
...
//     std::ofstream outputFile(local, std::ios::binary);
//     if (!outputFile.is_open())
//     {
//       std::cerr << "Failed to create local file: " << local << std::endl;
//       return false;
//     }
...
//     if (res != CURLE_OK)
//     {
//       std::cerr << "Failed to download CSV file. Error: " << curl_easy_strerror(res) << std::endl;
//       outputFile.close();
...
//     outputFile.close();
//     curl_easy_cleanup(curl);
//     std::cout << "CSV file downloaded successfully." << std::endl;
//     return true;

//     std::cout << "I DOWNLOAD" << std::endl;
// };

bool HttpRequest::download(const std::string &url, const std::string &local) const
{
    CURL* curl = curl_easy_init();
    if (!curl)
    {
