...
void HttpServer::send_file(int sock_fd_, const std::string &filename)
{
  HttpRequest request;
  std::fstream file;
    file.open(request.GetFileName(), std::ios_base::in | std::ios_base::binary);
    if (file.is_open())
    {
      int file_size = std::filesystem::file_size(request.GetFileName()) + 1;
      char *bytes = new char[file_size];
      file.read(bytes, file_size);

      send(sock_fd_, std::to_string(file_size).c_str(), 16, 0);
      send(sock_fd_, request.GetFileName().c_str(), 32, 0);
      send(sock_fd_, bytes, file_size, 0);
...
void HttpServer::Listen() {
  ...
  // принимает новые подключения и распределяет задачи по рабочим потокам
  while (running_) {
    if (!active) {
